#!/bin/bash -l
#SBATCH --job-name=genoa2-L40
#SBATCH --nodes=1
#SBATCH -w genoa2                
###### SBATCH --gres=gpu:L40:1
#SBATCH --time=24:00:00
#SBATCH --reservation=powerbench-GPU
#SBATCH --output=genoa2-L40-%j.out
#SBATCH --export=ALL

# === Load modules ===
export MODULESHOME=/apps/modules/5.1.1
module load cuda nvhpc gromacs/2024.3-gcc-mkl-cuda11.7 #gromacs/2024.4 #gromacs/2024.3-gcc-mkl-cuda11.7 # gromacs/2024.3-gcc-mkl-cuda11.7 (testcluster) gromacs/2023.3-gcc11.4.0-openmpi-openblas-fftw-cuda-h5ojrrx (gracehop1) # module load gromacs/2024.2-gcc-mkl-cuda11.7 # for testcluster gromacs/2024.4-gcc11.2.0-mkl-cuda (alex)
module avail 
...
OUTDIR=Host${HOSTNAME}-PowerCAP${POWER_LIMIT}W-ID${SLURM_JOB_ID}
mkdir -p $OUTDIR
cd $OUTDIR
...
for i in "${BENCHMARKS[@]}"; do
    ...
    # Main GROMACS run
    gmx mdrun -nsteps $NSTEPS $OPTIONS -s ${INPUTFILE_LOCATION}$i.tpr -deffnm ${i}_${POWER_LIMIT}_${HOSTNAME}_perflog  -g ${i}_${POWER_LIMIT}_${HOSTNAME}_perflog.log -gpu_id $CUDA_VISIBLE_DEVICES > ${i}_${POWER_LIMIT}_${HOSTNAME}_perflog.stdout
